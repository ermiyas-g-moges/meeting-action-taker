...
export default function VideoArchives({ refreshTrigger }) {
  ...
  const handleDeleteVideo = async (fileName, e) => {
    ...
    if (window.confirm(`Are you sure you want to delete "${fileName}"?`)) {
      try {
        await invoke('delete_recording', { name: fileName });
        if (selectedVideo && selectedVideo.includes(fileName)) {
              ...
              {recordings.map((file) => (
                <li key={file} className="group relative">
                  <button
                    ...
                    className="w-full rounded-lg px-3 py-2 text-left text-sm transition hover:bg-[var(--panel-muted)] hover:text-[var(--accent)] pr-10"
                  >
                    <div className="truncate font-medium">{file}</div>
                    <div className="text-[10px] text-[var(--text-soft)]">Video Recording</div>
                  </button>
                  <button
                    onClick={(e) => handleDeleteVideo(file, e)}
                    className="absolute right-2 top-1/2 -translate-y-1/2 rounded-md p-1.5 text-[var(--text-soft)] opacity-0 transition hover:bg-red-500/10 hover:text-red-500 group-hover:opacity-100"
